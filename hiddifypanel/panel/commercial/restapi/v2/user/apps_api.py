...
class AppSchema(Schema):
    ...
    description = String(
        required=True, description="The description of the app"
    )
    icon_url = URL(required=True, description="The icon url of the app")
    guide_url = URL(description="The guide url of the app")
    deeplink = URL(
        required=True, description="The deeplink of the app to imoprt configs"
    )
    install = List(
        Nested(AppInstall()),
        required=True,
...
class AppAPI(MethodView):
    ...
    def __init__(self) -> None:
        ...
        self.user_panel_encoded_url = hutils.encode.url_encode(
            self.user_panel_url
        )
        ...
        self.subscription_link_encoded_url = hutils.encode.do_base_64(
            self.subscription_link_url
        )
    ...
    def get(self, data):
        ...
        match self.platform:
            ...
            case Platform.android:
                ...
                apps_data += [hiddify_next_dto, happ_dto]
    ...
    def __get_ua_platform(self):
        ...
        if os == "Android":
            ...
            "Linux" in request.user_agent.string
            and "X11"
            or "Wayland" in request.user_agent.string
        ):
    ...
    def __get_app_icon_url(self, app_name):
        ...
        if app_name == _("app.hiddify.next.title"):
            ...
            url = base + static_url_for(
                filename="apps-icon/clash_verge_rev.ico"
            )
            ...
            url = base + static_url_for(
                filename="apps-icon/hiddify_android.ico"
            )
    ...
    def __get_app_install_dto(
        self, install_type: AppInstallType, url, title=""
    ):
    ...
    def __get_v2rayn_app_dto(self):
        ...
        dto.deeplink = (
            f"v2rayn://install-sub/?url={self.user_panel_encoded_url}"
        )

        ins_url = f"https://github.com/2dust/v2rayN/releases/latest/download/v2rayN-windows-64-SelfContained-With-Core.7z"
        dto.install = [
            self.__get_app_install_dto(AppInstallType.portable, ins_url)
        ]
        return dto
    ...
    def __get_nekobox_app_dto(self):
        ...
        dto.guide_url = ""
        dto.deeplink = (
            f"clash://install-config?url={self.user_panel_encoded_url}"
        )

        latest_url, version = get_latest_release_url(
            f"https://github.com/MatsuriDayo/NekoBoxForAndroid"
        )
        ins_url = (
            latest_url.split("releases/")[0]
            + f"releases/download/{version}/NB4A-{version}-x86_64.apk"
        )
    ...
    def __get_v2raytun_app_dto(self):
        dto = AppSchema()
        dto.title = "V2rayTun"
        dto.description = "V2rayTun"
        base = f"https://{urlparse(request.base_url).hostname}"
        dto.icon_url = base + static_url_for(filename="apps-icon/v2raytun.ico")
        dto.guide_url = ""
        dto.deeplink = f"v2raytun://import/{self.subscription_link_url}"
        dto.install = []
        return dto
    ...
    def __get_happ_app_dto(self):
        dto = AppSchema()
        dto.title = "Happ"
        dto.description = "Happ"
        base = f"https://{urlparse(request.base_url).hostname}"
        dto.icon_url = base + static_url_for(filename="apps-icon/happ.ico")
        dto.guide_url = ""
        dto.deeplink = f"happ://add/{self.subscription_link_url}"
        dto.install = []
        return dto
    ...
    def __get_v2rayng_app_dto(self):
        ...
        dto.deeplink = (
            f"v2rayng://install-sub/?url={self.user_panel_encoded_url}"
        )

        # make v2rayng latest version url download
        latest_url, version = get_latest_release_url(
            f"https://github.com/2dust/v2rayNG/"
        )
        github_ins_url = (
            latest_url.split("releases/")[0]
            + f"releases/download/{version}/v2rayNG_{version}_universal.apk"
        )
        google_play_ins_url = (
            "https://play.google.com/store/apps/details?id=com.v2ray.ang"
        )
        dto.install = [
            self.__get_app_install_dto(AppInstallType.apk, github_ins_url),
            self.__get_app_install_dto(
                AppInstallType.google_play, google_play_ins_url
            ),
        ]
        return dto
    ...
    def __get_foxray_app_dto(self):
        ...
        dto.install = [
            self.__get_app_install_dto(AppInstallType.app_store, ins_url),
        ]
        return dto
    ...
    def __get_streisand_app_dto(self):
        ...
        dto.deeplink = (
            f"streisand://import/{self.user_panel_url}#{self.profile_title}"
        )
    ...
    def __get_hiddify_clash_desktop_app_dto(self):
        ...
        dto.icon_url = self.__get_app_icon_url(
            _("app.hiddify-clash-desktop.title")
        )
        ...
        latest_url, version = get_latest_release_url(
            f"{self.hiddify_github_repo}/hiddifydesktop"
        )
        ...
        def get_link(p):
            match p:
                case Platform.windows:
                    ins_url = (
                        latest_url.split("releases/")[0]
                        + f"releases/download/v{version}/HiddifyClashDesktop_{version}_x64_en-US.msi"
                    )
                    dto.install.append(
                        self.__get_app_install_dto(
                            AppInstallType.setup, ins_url
                        )
                    )
                case Platform.linux:
                    ins_url = (
                        latest_url.split("releases/")[0]
                        + f"releases/download/v{version}/hiddify-clash-desktop_{version}_amd64.AppImage"
                    )
                    dto.install.append(
                        self.__get_app_install_dto(
                            AppInstallType.appimage, ins_url
                        )
                    )
                case Platform.mac:
                    ins_url = (
                        latest_url.split("releases/")[0]
                        + f"releases/download/v{version}/HiddifyClashDesktop_{version}_x64.dmg"
                    )
                    dto.install.append(
                        self.__get_app_install_dto(AppInstallType.dmg, ins_url)
                    )
    ...
    def __get_singbox_app_dto(self):
        ...
        if self.platform == Platform.all:
            installation_types = [
                AppInstallType.apk,
                AppInstallType.google_play,
                AppInstallType.dmg,
                AppInstallType.app_store,
            ]
        else:
            match self.platform:
                case Platform.android:
        ...
        for install_type in installation_types:
            ...
            match install_type:
                case AppInstallType.apk:
                    latest_url, version = get_latest_release_url(
                        f"https://github.com/SagerNet/sing-box"
                    )
                    ins_url = (
                        latest_url.split("releases/")[0]
                        + f"releases/download/{version}/SFA-{version}-universal.apk"
                    )
                ...
                case AppInstallType.dmg:
                    ...
                    ins_url = (
                        latest_url.split("releases/")[0]
                        + f"releases/download/{version}/SFM-{version}-universal.dmg"
                    )
    ...
    def __get_clash_verge_rev_app_dto(self):
        ...
        if self.platform == Platform.all:
            installation_types = [
                AppInstallType.setup,
                AppInstallType.appimage,
                AppInstallType.dmg,
            ]
        else:
            match self.platform:
        ...
        for install_type in installation_types:
            ...
            match install_type:
                case AppInstallType.appimage:
                    ins_url = (
                        "https://www.clashverge.dev/install.html#__tabbed_1_2"
                    )
                case AppInstallType.setup:
                    ins_url = (
                        "https://www.clashverge.dev/install.html#__tabbed_1_1"
                    )
                case AppInstallType.dmg:
                    ins_url = (
                        "https://www.clashverge.dev/install.html#__tabbed_1_3"
                    )
    ...
    def __get_cmfa_app_dto(self):
        ...
        latest_url, version = get_latest_release_url(
            f"https://github.com/MetaCubeX/ClashMetaForAndroid"
        )
        ins_url = (
            latest_url.split("releases/")[0]
            + f"releases/download/{version}/cmfa-{version}-meta-universal-release.apk"
        )
